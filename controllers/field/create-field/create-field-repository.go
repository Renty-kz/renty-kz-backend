...
func (r *repository) CreateFieldRepository(input *model.EntityFields) (*model.EntityFields, string) {
	...
	checkOrganizationActiveness := r.db.Model(&organizations).Debug().Select("*").Where("id = ?", input.OrganizationID).Find(&organizations)
	if checkOrganizationActiveness.RowsAffected < 1 {
